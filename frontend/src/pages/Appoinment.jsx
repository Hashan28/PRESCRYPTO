    ...
    <div>
      ...
      <div className='flex flex-col sm:flex-row gap-4'>
        ...
        <div className='flex-1 border border-grey-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0'>
          {/* doc info name digree experience */}
          ...
          {/* doctor about */}
          <div>
            <p className='flex items-center gap-1 text-sm font-medium text-grey-900 mt-3'>
              About <img src={assets.info_icon} alt="" />
            </p>
            <p className='text-sm text-grey-600 max-w-[700px] mt-1'>{docInfo.about}</p>
          </div>
          <p className='text-grey-500 font-medium mt-4'>
            Appointment fee: <span className='text-grey-600'>{currencySymbols}{docInfo.fees}</span>
          </p>
        </div>
      </div>
      {/* bookin slots */}
      <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-grey-700'>
        ...
        <div className='flex gap-3 items-center w-full overflow-x-scroll mt-4'>
          {
            docSlots.length && docSlots.map((item, index) => (
              <div onClick={()=> setSlotIndex(index)} className={`text-center py-6 min-w-16 rounded-full cursor-pointer ${slotIndex === index ? 'bg-primary text-white' : 'border border-grey-200'}`} key={index}>
        ...
        <button className='bg-primary text-white text-sm font-light px-14 py-3 rounded-full mt-7'>Book an appointment</button>
      </div>
